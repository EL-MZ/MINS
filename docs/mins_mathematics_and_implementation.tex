...
\definecolor{minsblue}{HTML}{164E63}
\definecolor{minsgreen}{HTML}{166534}
\definecolor{minsred}{HTML}{991B1B}
\definecolor{minsgray}{HTML}{F3F4F6}
...
\hypersetup{
  ...
  urlcolor=minsblue,
  citecolor=minsblue,
  pdftitle={MINS: Mathematics, Implementation, API Adaptation, and Termination Rules},
  pdfauthor={MINS contributors}
}

\lstdefinestyle{minspython}{
  language=Python,
  basicstyle=\ttfamily\small,
  keywordstyle=\color{minsblue}\bfseries,
  commentstyle=\color{minsgreen},
  stringstyle=\color{minsred},
  numbers=left,
  numberstyle=\scriptsize\color{codegray},
  numbersep=8pt,
  backgroundcolor=\color{minsgray},
  frame=single,
...
\lstset{style=minspython}

\newcommand{\MINS}{\textsc{MINS}}
...
\pagestyle{fancy}
\fancyhf{}
\lhead{\MINS{} mathematics and implementation}
\rhead{\thepage}
...
\author{Implementation guide for \code{mins} version \code{0.1.0.dev3}}
...
Morph-assisted nested importance sampling (\MINS) code and connects each
...
\MINS{} is a \emph{post-processing evidence estimator}.  The user first
...
\subsection{Fixed Morph proposal and pseudo-likelihood}

The posterior training array is used to fit one normalized proposal density
\(q(\theta)\).  In the standard path, \(q\) is a \MorphZ{}
...
\begin{equation}
  \Psi(\theta)
  = \frac{L(\theta)\pi(\theta)}{q(\theta)},\qquad
  \log\Psi(\theta)
...
\begin{equation}
  Z
  = \int_{\Theta}\Psi(\theta)q(\theta)\dd\theta.
  \label{eq:change-measure}
\end{equation}
Thus \(q\) acts as the nested sampler's pseudo-prior and \(\Psi\) acts as its
pseudo-likelihood.  No additional Metropolis--Hastings ratio or later
importance correction is applied.  This implementation fixes any tempering
...
\(\log L+\log\pi\) is finite but \(\log q=-\infty\), it raises
...
\code{MINSResult.resample_equal} uses randomized systematic resampling.  For a
...
This section follows \code{MINSampler.run} in execution order.

\subsection{Step 0: resolve and validate configuration}

\code{MINSConfig} is immutable and validates all settings before sampling.
If neither \code{dlogz} nor \code{stopping} is supplied, the configuration is
resolved to \code{dlogz=1e-3}.  A legacy \code{dlogz} value is internally
represented as a one-criterion \code{StoppingPolicy} using
\code{remaining_fraction}.  Supplying both interfaces is an error.
...
Use when a grouping definition was computed previously.  MINS reads it and
...
\code{MINSampler.from_posterior_samples} is a convenience path.  Every item in
...
proposal = UniformBoxProposal()
sampler = MINSampler(model, proposal, n_live=500, rng=42)
\end{lstlisting}
...
Explicit value must satisfy \(0<\code{dlogz}<1\). &
Use for the legacy single \code{remaining_fraction} scientific rule.  Smaller
values demand a smaller live contribution and generally run longer.  This is
not a direct absolute error tolerance.\\
\code{stopping} &
...
construct \code{MINSampler} directly and omit Morph fitting arguments.\\
More stringent legacy completion &
...
\]
passes at equality.  Its tolerance must satisfy \(0<\tau<1\).  The legacy API
\begin{lstlisting}
result = sampler.run(dlogz=1.0e-3)
\end{lstlisting}
is exactly a one-criterion policy with
...
\important{\(f_{\mathrm{live}}\) measures the fraction of the current evidence
estimate assigned to the live set.  It is not itself an estimate of
\(|\widehat{\log Z}-\log Z|\), and \code{dlogz} is therefore a historical API
name rather than the exact formula being compared.}
...
uncertainty, which is why using this rule with a remaining-fraction guard is
safer than using it alone.
...
These threshold values are initial calibration choices in the repository, not
universal constants.  They should be assessed using repeated seeds, known
benchmarks where possible, proposal diagnostics, and a live count appropriate
to the new target.
...
The run used the legacy/default \code{dlogz} interface, and after a completed
replacement its remaining-fraction policy reached the required streak.\\
...
\code{MINSResult} with the current final-live correction, but set
\code{success=False}:
...
\begin{lstlisting}
if not result.success:
    print("MINS did not converge scientifically:", result.termination_reason)
\end{lstlisting}
...
The result separately stores points, \(\log L\), \(\log\pi\), \(\log q\),
\(\log\Psi\), and tie breakers for dead and final-live populations.  Dead
...
\begin{itemize}
  ...
  \item remaining fraction, live ESS, live-mean RSE, live log-evidence error,
        stability, and stopping streak;
...
\code{progress=True} displays iteration, live count, likelihood calls,
efficiency, current evidence, theoretical error, finite-live diagnostics, live
ESS, and stopping streak.  A callable receives a mapping after every completed
iteration, including a \code{criterion_<name>_met} integer flag for every
enabled criterion.
...
\begin{enumerate}
  ...
  \item \textbf{Which scientific stop is intended?}  Use only \code{dlogz} for
        legacy remaining fraction, or only \code{stopping} for a policy.
