...
\hypersetup{
  ...
  urlcolor=minsblue,
  citecolor=minsblue,
  pdftitle={The Statistically Specified s-rwalk Implementation in MINS},
...
\pagestyle{fancy}
\fancyhf{}
\lhead{\MINS{} statistically specified \code{s-rwalk}}
\rhead{\thepage}
...
\code{proposal_scheme="s-rwalk"} kernel implemented by \MINS.  It follows the
...
\begin{itemize}
  \item \code{src/mins/config.py}: public \code{SRWalkSettings} and the
        \code{"s-rwalk"} proposal-scheme literal;
  \item \code{src/mins/mcmc.py}: \code{SRWalkSampler}, regularized covariance
        factor, Gaussian proposal evolution, fixed-\(q_0\) acceptance, and
        scale tuning;
  \item \code{src/mins/sampler.py}: controller lifetime, dispatch, resource
        integration, live-point replacement, and per-iteration histories;
  \item \code{src/mins/__init__.py}: public export of
        \code{SRWalkSettings}.
\end{itemize}
...
correctness of the target density.  Each call to \code{MINSampler.run()}
...
\MINS{} proposes in the full physical parameter space.  It does not clip,
...
Before choosing a start or constructing covariance geometry, \MINS{} rejects
...
\MINS{} does not force an accepted transition, discard unchanged outputs,
...
\code{MINSampler.run()} must be at least \code{n_steps}.
...
not required for detailed balance.  The \MINS{} fixed-importance target and
